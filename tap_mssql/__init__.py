...
REQUIRED_CONFIG_KEYS = ["host", "database", "user", "password", "tables"]
...
def discover_catalog(mssql_conn, config):
    ...
    filter_tables_config = config.get("tables")
    filter_tables_clause = ",".join(
        ["'{}'".format(tb.strip()) for tb in filter_tables_config.split(",")])
    tables_name_clause = "AND c.table_name IN ({})".format(
        filter_tables_clause)
    where_clause = table_schema_clause + " " + tables_name_clause
    ...
    with connect_with_backoff(mssql_conn) as open_conn:
        ...
        for (db, table, table_type) in cur.fetchall():
            ...
            table_info[db][table] = {
                "row_count": None, "is_view": table_type == "VIEW"}
...
def resolve_catalog(discovered_catalog, streams_to_sync):
    ...
    for catalog_entry in streams_to_sync:
        ...
        if not discovered_table:
            raise SymonException(f'Database {database_name} table {catalog_entry.table} was selected but does not exist.', 'mssql.TableNotFound')
...
def get_non_binlog_streams(mssql_conn, catalog, config, state):
    ...
    if currently_syncing:
        ...
        non_currently_syncing_streams = list(
            filter(lambda s: s.tap_stream_id !=
                   currently_syncing, ordered_streams)
        )
...
def do_sync_full_table(mssql_conn, config, catalog_entry, state, columns):
    ...
    full_table.sync_table(mssql_conn, config, catalog_entry,
...
def sync_non_binlog_streams(mssql_conn, non_binlog_catalog, config, state):
    ...
    for catalog_entry in non_binlog_catalog.streams:
        ...
        if not columns:
            raise SymonException(f'There are no columns selected for stream {catalog_entry.stream}.', 'mssql.EmptyData')
        ...
        with metrics.job_timer("sync_table") as timer:
            ...
            if replication_method == "INCREMENTAL":
                ...
                do_sync_incremental(mssql_conn, config,
                                    catalog_entry, state, columns)
            elif replication_method == "FULL_TABLE":
                LOGGER.info(f"syncing {catalog_entry.table} full table")
                do_sync_full_table(mssql_conn, config,
                                   catalog_entry, state, columns)
            else:
                raise Exception(
...
def log_server_params(mssql_conn):
    with connect_with_backoff(mssql_conn) as open_conn:
        try:
            ...
            LOGGER.warning(
                "Encountered error checking server params. Error: (%s) %s", *e.args)
